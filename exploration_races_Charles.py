...
def min_max_pit_stop_drivers(
    ...
    ----------
    nom_pilote : str
        Identifiant du pilote (champ `driverRef` dans le DataFrame `drivers`).
    ...
    -------
    pd.DataFrame
        Un DataFrame avec une seule ligne contenant la moyenne des temps d'arrêt 
        correspondants (min ou max) pour le pilote spécifié. La colonne retournée sera 
        `min_pit_stop` ou `max_pit_stop` selon la valeur de `param`.
...
def generer_table_fichier(nom_fichier_recherche):
    ...
    nom_fichier_recherche : str
        Le nom du fichier (sans extension) à rechercher dans le dossier.
    ...
    ----------
    ValueError :
        Si le dossier 'donnees_formule_un' n'existe pas sur le bureau de l'utilisateur.
...
def nbr_victoire_joueurs():
    ...
    -------
    dict
        Un dictionnaire où les clés sont les identifiants des pilotes
...
def nbr_points_joueurs():
    ...
    -------
    dict
        Un dictionnaire dont les clés sont les identifiants des pilotes,
...
def nbr_victoire_ttal_pilote():
    ...
    -------
    dict
        Un dictionnaire avec les noms des pilotes comme clés et leur nombre total de victoires comme valeurs.
